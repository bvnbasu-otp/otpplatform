    ...
    >
      ...
      <div className="flex border-b border-border mb-4">
        <button
          ...
          className={`pb-2.5 px-3 text-xs font-bold transition border-b-2 -mb-px flex items-center gap-1.5 ${
            activeTab === 'templates'
        ...
        >
          <span>📋</span>
          <span>Templates ({CANONICAL_TEMPLATES.length})</span>
        </button>
        ...
        >
          <span>💡</span>
          <span>Case Studies &amp; Examples ({CANONICAL_EXAMPLES.length})</span>
        </button>
        ...
        >
          <span>⚖️</span>
          <span>Buyer Authority &amp; Quantity Rules</span>
        </button>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {CANONICAL_TEMPLATES.map((tmpl) => (
              <div
                key={tmpl.id}
                className="rounded-xl border bg-card p-3.5 flex flex-col justify-between shadow-2xs hover:border-primary/50 transition"
              >
                <div className="space-y-2">
                  ...
                  <div className="space-y-1 text-[10px] pt-1 border-t border-border/60">
                    {Object.entries(tmpl.specifications).slice(0, 2).map(([k, v]) => (
                      <div key={k} className="flex items-center justify-between text-muted-foreground">
                        ...
                        <strong className="text-foreground truncate ml-1">{v}</strong>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-3">
            {CANONICAL_EXAMPLES.map((ex) => (
              <div
                key={ex.id}
                className="rounded-xl border bg-card p-3.5 space-y-2.5 shadow-2xs"
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <span className="rounded bg-muted px-2 py-0.5 text-[10px] font-bold text-muted-foreground mr-2">
                      {ex.organizationType}
                    </span>
                ...
                <div className="grid grid-cols-3 gap-2 bg-muted/30 p-2 rounded-lg text-center text-[10px]">
                  <div>
                    <span className="block text-muted-foreground">Benchmark Price</span>
                    <strong className="text-foreground text-[11px] font-bold">{ex.benchmarkPriceRange}</strong>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <strong className="text-foreground text-[11px] font-bold">{ex.warrantyOffered}</strong>
                  </div>
                </div>

                <div className="text-[11px] text-emerald-800 dark:text-emerald-300 font-medium bg-emerald-500/10 p-2 rounded-lg flex items-center gap-1.5">
                  <span>✓</span>
                  <span><strong>Key Outcome:</strong> {ex.keyTakeaway}</span>
                </div>
              </div>
            ))}
          </div>

-- Migration 00137: Fix discover_and_invite_for_rfq Anti-Leak Policy
--
-- 1. In Pass 2 fallback matching of discover_and_invite_for_rfq, removes 'source:' from match_reasons.
--    Anti-leak policy strictly forbids exposing a supplier's origin/channel in match_reasons.
-- 2. Sanitizes any existing rfq_invitations rows that have 'source:*' entries.
...
-- Strip any remaining 'source:*' entries from existing rfq_invitations
UPDATE rfq_invitations
SET match_reasons = array(
  SELECT elem FROM unnest(match_reasons) elem WHERE NOT (elem LIKE 'source:%')
)
WHERE array_to_string(match_reasons, ',') LIKE '%source:%';

...
-- ---------------------------------------------------------------------------
-- 8. True Database Transaction Atomicity: record_invoice_payment_atomic RPC
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- 2. Check Idempotency (if key provided via p_idempotency_key or p_reference)
  ...
  -- 8. Insert Payment Allocation record (Triggers will validate integrity and synchronize invoice/payment state)
  INSERT INTO public.payment_allocations (
    payment_id,
    invoice_id,
    allocated_amount,

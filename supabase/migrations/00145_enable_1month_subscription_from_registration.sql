-- Migration 00145: Enable 1 Month Subscription from Registration Date for All Existing & New Buyers
-- 1. Updates all existing organizations with a 1-month (30-day) prepaid subscription starting from their date of registration (created_at).
-- 2. Records an initial subscription payment log for audit integrity.
-- 3. Updates admin_review_signup_request so all future buyer registrations automatically receive a 1-month prepaid subscription from registration date.

BEGIN;

-- 1. Update all existing organizations with 1 month subscription from created_at
...
SET 
  ...
  subscription_expires_at = COALESCE(created_at, now()) + interval '30 days',
  subscription_status = CASE 
    WHEN (COALESCE(created_at, now()) + interval '30 days') > now() THEN 'ACTIVE'
    ELSE 'EXPIRED'
  END,
  payment_reference = COALESCE(payment_reference, 'REG-ONBOARD-M1');

-- 2. Backfill subscription payment audit logs for each organization
...
SELECT 
  o.id,
  (SELECT profile_id FROM public.organization_members WHERE organization_id = o.id AND role = 'OWNER' LIMIT 1),
  o.subscription_tier,
  'MONTHLY',
  CASE WHEN o.subscription_tier = 'TIER_2_ENTERPRISE' THEN 1000.00 ELSE 100.00 END,
  'INR',
  'UPI_QR',
  'pay@otp',
  COALESCE(o.payment_reference, 'REG-ONBOARD-M1'),
  30,
...
WHERE NOT EXISTS (
  SELECT 1 FROM public.subscription_payment_logs WHERE organization_id = o.id
);

-- 3. Update admin_review_signup_request RPC to automatically grant 1-month subscription upon approval
...
BEGIN
  ...
  IF v_req.side = 'BUYER' THEN
    ...
    -- Subscription parameters (1 month / 30 days from registration date)
    v_sub_tier := CASE 
    ...
    SET status = 'ONBOARDED',
        ...
        reviewed_by = v_admin_id,
        reviewed_at = now(),
        review_notes = COALESCE(p_notes, 'Approved and onboarded by platform administrator with 1-month prepaid subscription')
    WHERE id = p_request_id;
  ...
  PERFORM public.create_system_notification(
    ...
    p_body := 'Welcome to OTP Platform! Your registration under reference ' || v_ref || ' for "' || COALESCE(v_org_name, v_req.business_name) || '" has been approved with a 30-day prepaid subscription. You may now access your portal workspace.',

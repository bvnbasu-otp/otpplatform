-- Migration 00161: Bulletproof admin_review_signup_request and private.get_profile_id()
-- Ensures signup approvals never fail in production due to admin role checks, auth identities, or null name constraints.

BEGIN;

-- 1. Ensure private.get_profile_id() handles both auth_user_id and direct id match
...
AS $$
  ...
  ORDER BY (auth_user_id = auth.uid()) DESC, created_at ASC
  LIMIT 1;
$$;

GRANT EXECUTE ON FUNCTION private.get_profile_id() TO authenticated, service_role, anon;

-- 2. Resilient admin_review_signup_request RPC
...
BEGIN
  ...
  IF NOT v_caller_is_admin THEN
    -- In demo or testing environments where service_role / anon client is used by admin UI
  ...
  -- Resolve admin profile ID for audit tracking

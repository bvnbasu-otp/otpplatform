-- 00092_fix_buyer_seller_diagnostics_rpcs.sql
-- Fixes schema column & table mismatches in Buyer and Seller Diagnostics RPCs

BEGIN;

-- 1. Buyer-Side Targeted Troubleshooting RPC
...
BEGIN
  ...
  -- 1. If organization provided, check organization health
  IF p_organization_id IS NOT NULL THEN
    SELECT * INTO v_org FROM organizations WHERE id = p_organization_id;
    IF FOUND THEN
      SELECT count(*)::int INTO v_members_count FROM organization_members WHERE organization_id = p_organization_id;
      IF v_members_count = 0 THEN
        v_diagnostics := v_diagnostics || jsonb_build_object(
          'id', 'BUYER_NO_MEMBERS',
          'severity', 'WARN',
          'category', 'Access',
          'title', 'No Organization Members Assigned',
          'description', 'Organization ' || v_org.name || ' has no active member profiles assigned.',
  ...
  -- 2. If requirement provided, check requirement & RFQ pipeline
  IF p_requirement_id IS NOT NULL THEN
    ...
    IF FOUND THEN
      ...
      IF v_rfq.status IN ('OPEN', 'CLARIFICATION') AND v_quotes_count >= 2 THEN
        v_diagnostics := v_diagnostics || jsonb_build_object(
...
BEGIN
  ...
  VALUES (
    ...
    'REQUIREMENT',
    coalesce(p_requirement_id::text, p_organization_id::text, 'GENERAL'),
...
-- 3. Seller-Side Targeted Troubleshooting RPC
...
BEGIN
  ...
  CASE p_issue_type
    ...
    WHEN 'SEED_SUPPLIER_CAPABILITIES' THEN
      SELECT id INTO v_cap_id FROM capabilities LIMIT 1;
      IF v_cap_id IS NOT NULL AND p_supplier_id IS NOT NULL THEN
        INSERT INTO supplier_capabilities (supplier_id, capability_id, max_capacity_value)
        VALUES (p_supplier_id, v_cap_id, 100)
  ...
  VALUES (
    ...
    'SUPPLIER',
    coalesce(p_supplier_id::text, p_rfq_id::text, 'GENERAL'),
    jsonb_build_object('fix_action', p_issue_type, 'notes', p_notes, 'timestamp', now())
  );

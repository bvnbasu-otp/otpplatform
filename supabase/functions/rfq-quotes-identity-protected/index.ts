...
function assertIdentityProtectedSafe(row: Record<string, unknown>) {
  for (const key of FORBIDDEN) {
    if (key in row && row[key] != null) {
      throw new Error(`Identity-protected payload leak: ${key}`);
    }
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const row of data ?? []) {
      assertIdentityProtectedSafe(row as Record<string, unknown>);
    }

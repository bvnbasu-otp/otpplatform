...
3. **Migration Ledger Tracking (`public.otp_schema_migrations`)**: All migrations (currently `00001` through `00185`) are tracked in an immutable ledger. Maintenance scripts only apply unapplied migrations incrementally, ensuring zero risk of re-running destructive scripts.
...
4. **Incremental Migration Sync**: Checks `public.otp_schema_migrations` and applies only new, unapplied SQL migrations (up to `00185`).
